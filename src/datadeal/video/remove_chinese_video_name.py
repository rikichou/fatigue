...
def deal(item):
    ...
    # aitxt_path = prefix+'.aitxt'
    # if not os.path.exists(aitxt_path):
    #     print("Can not find ", aitxt_path)
    #     return 1
    ...
    if new_name != name:
        ...
        if os.path.isdir(f):
            ...
            new_video_name = os.path.join(os.path.dirname(f), basename)+ext
            new_aitxt_name = os.path.join(os.path.dirname(f), basename)+'.'+'aitxt'

            print("rename: {} ---> {}".format(f, new_video_name))
            # print("rename: {} ---> {}".format(aitxt_path, new_aitxt_name))
