...
def extract_frame(vid_item):
    ...
    if str(Path('/')) in vid_path:
        out_full_path = osp.join(
            args.out_dir,
            vid_path.rsplit(str(Path('/')), maxsplit=1)[0])
    else:
    ...
    for i, vr_frame in enumerate(vr):
        if vr_frame is not None:
            ...
            im = Image.fromarray(np.uint8(vr_frame))

            im.save(os.path.join(out_full_path,
                                 'img_{:05d}.jpg'.format(i + 1)))
        else:
            warnings.warn('Length inconsistent!'
...
def parse_args():
    ...
    parser.add_argument('--mixed-ext',
                        action='store_true',
                        help='process video files with mixed extensions')
    parser.add_argument('--new-width',
                        type=int,
                        default=0,
                        help='resize image width')
    parser.add_argument('--new-height',
                        type=int,
                        default=0,
                        help='resize image height')
    parser.add_argument('--new-short',
                        type=int,
                        default=0,
...
if __name__ == '__main__':
    ...
    # BUG: part of fatigue dataset
    # fullpath_list = fullpath_list[:30]
    print('Total number of videos found: ', len(fullpath_list))

    if args.level == 3:
        vid_list = list(
            map(
                lambda p: os.path.join(*(p.rsplit(str(Path('/')), maxsplit=3)[
                    -3:])), fullpath_list))
    elif args.level == 1:
