...
def parse_directory(frame_dirs,
                    rgb_prefix='img_',
    ...
    for i, frame_dir in enumerate(frame_dirs):
        ...
        if i % 1000 == 0:
            print("{}/{} parsed".format(i+1, len(frame_dirs)))
...
def multi_process(frame_dirs, args):
    ...
    fp = open(osp.join(out_path, filename), 'w')
...
def main():
    ...
    # get all videos dir
    if level == 1:
        frame_dirs = glob.glob(osp.join(args.src_folder_frames, '*'))
    elif level == 2:
        frame_dirs = glob.glob(osp.join(args.src_folder_frames, '*', '*'))
    elif level == 3:
        frame_dirs = glob.glob(osp.join(args.src_folder_frames, '*', '*', '*'))
