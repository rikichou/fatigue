        ...
        for i, vr_frame in enumerate(vr):
            if vr_frame is not None:
                ...
                if len(bbox) != 4 or sum(bbox) < 1:
                    ...
                    sx, sy, ex, ey = bbox
                    face_image = get_input_face(image, [sx,sy,ex,ey])
                    face_image = cv2.resize(face_image, (args.size, args.size))

                    cv2.imwrite(os.path.join(args.out_dir, os.path.splitext(os.path.basename(v))[0]+'.jpg'), face_image)
                    break
            else:
                print(
                    'Length inconsistent!'
